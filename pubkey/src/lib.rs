...
impl Pubkey {
    ...
    pub fn new_unique() -> Self {
        use solana_hash::atomic_u64::AtomicU64;
    ...
    /// Find a valid [program derived address][pda] and its corresponding bump
    /// seed.
    ///
    ...
    ///
    /// **Warning**: Because of the way the seeds are hashed there is a
    /// potential for program address collisions for the same program id.
    /// The seeds are hashed sequentially which means that seeds {"abcdef"},
    /// {"abc", "def"}, and {"ab", "cd", "ef"} will all result in the same
    /// program address given the same program id. Since the chance of
    /// collision is local to a given program id, the developer of that
    /// program must take care to choose seeds that do not collide with each
    /// other. For seed schemes that are susceptible to this type of hash
    /// collision, a common remedy is to insert separators between seeds,
    /// e.g. transforming {"abc", "def"} into {"abc", "-", "def"}.
    ///
    /// # Panics
    ///
    /// Panics in the statistically improbable event that a bump seed could not
    /// be found. Use [`try_find_program_address`] to handle this case.
    ///
    /// [`try_find_program_address`]: Pubkey::try_find_program_address
    ///
    /// Panics if any of the following are true:
    ///
    /// - the number of provided seeds is greater than, _or equal to_,
    ///   [`MAX_SEEDS`],
    ...
    ///
    /// See the documentation for [`find_program_address`] for a full
    /// description.
    ///
    ...
    /// Create a valid [program derived address][pda] without searching for a
    /// bump seed.
    ///
    /// [pda]: https://solana.com/docs/core/cpi#program-derived-addresses
    ///
    ...
    ///
    /// However, it can be used for efficiently verifying that a set of seeds
    /// plus bump seed generated by [`find_program_address`] derives a
    /// particular address as expected. See the example for details.
    ///
    /// See the documentation for [`find_program_address`] for a full
    /// description of program derived addresses and bump seeds.
    ///
    /// [`find_program_address`]: Pubkey::find_program_address
    ///
    /// # Examples
    ...
    pub fn create_program_address(
        ...
        #[cfg(not(target_os = "solana"))]
        {
            let mut hasher = solana_hash::sha256::Hasher::default();
            for seed in seeds.iter() {
                hasher.hash(seed);
            }
