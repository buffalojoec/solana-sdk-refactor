//! Implementations of syscalls used when `solana-program` is built for non-SBF
//! targets.
...
use {
    ...
    itertools::Itertools,
    solana_builtin_errors::UNSUPPORTED_SYSVAR,
...
{
    // If the absolute distance between the ptrs is at least as big as the size of
    // the other, they do not overlap.
    if src > dst {
