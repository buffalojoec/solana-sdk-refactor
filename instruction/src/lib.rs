...
use {
    ...
    solana_builtin_errors::{
        ACCOUNT_ALREADY_INITIALIZED, ACCOUNT_BORROW_FAILED, ACCOUNT_DATA_TOO_SMALL,
        ACCOUNT_NOT_RENT_EXEMPT, ARITHMETIC_OVERFLOW, BORSH_IO_ERROR, BUILTIN_BIT_SHIFT,
...
/// - The transaction's fee-paying account &mdash; it is added during
///   [`Message`] construction. A program may still require the fee payer as
///   part of the account list if it directly references it.
///
...
impl Instruction {
    ...
    /// [`borsh`]: https://docs.rs/borsh/latest/borsh/
    ///
    /// `program_id` is the address of the program that will execute the
    /// instruction. `accounts` contains a description of all accounts that
    /// may be accessed by the program.
    ///
    ...
    /// may be accessed by the program.
    ///
    /// The caller is responsible for ensuring the correct encoding of `data` as
    /// expected by the callee program.
    ///
    /// # Examples
    ///
    /// ```
